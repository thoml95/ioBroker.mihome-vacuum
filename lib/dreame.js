...
class DreameManager {
	constructor(adapterInstance, Miio, miApi) {
		...
		let mapdata = [{"siid": 6, "aiid": 1}];
		this.Miio.sendMessage('{"siid": 6, "aiid": 1}', [{'piid': 2, 'value': '{"req_type":1,"frame_type":"I","force_type":1}'}]).then((result) => {
	...
	async getStates() {
		...
		let DeviceData;
				...
				adapter.log.debug('getting MAP DATA');
			}
...
async getMapPointer() {
		...
		try {
			for (let index = 0; index < 5; index++) {
				let data = [{ did: "", siid: DreameProperties.MAP_DATA.siid, piid: DreameProperties.MAP_DATA.piid }];
				let answer = await this.Miio.sendMessage("action", parameters={"did": f'{6}.{1}',"siid": 6,"aiid": 1,"in": [{'piid': 2, 'value': '{"req_type":1,"frame_type":"I","force_type":1}'}]});
				adapter.log.debug(JSON.stringify(answer));
				if (answer.result) {
